  ...
  return (
    <div className="p-4 lg:p-6 pb-24">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {[
          { label: "Total", value: assignments.length, color: "text-slate-700", bg: "bg-slate-100" },
          { label: "Pending", value: pending.length, color: "text-yellow-700", bg: "bg-yellow-100" },
          { label: "Submitted", value: submitted.length, color: "text-blue-700", bg: "bg-blue-100" },
          { label: "Overdue", value: overdue.length, color: "text-red-700", bg: "bg-red-100" },
        ].map((s, i) => (
          <motion.div
            key={s.label}
      ...
      <Tabs defaultValue="all">
        <TabsList className="bg-white border border-slate-200 rounded-xl p-1 mb-5 gap-1">
            ...
            {(tab === "all" ? assignments : tab === "pending" ? pending : challenges).map((assignment, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className={cn(
                    "bg-white rounded-[14px] border p-4 hover:shadow-md transition-all duration-200",
                    assignment.type === "challenge" ? "border-orange-200 bg-gradient-to-r from-orange-50/50 to-white" : "border-slate-100"
                  )}
                >
                  <div className="flex items-start gap-3">
                    <div className={cn(
                      "w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5",
                      assignment.type === "challenge" ? "bg-orange-100" : "bg-purple-100"
                    )}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 flex-wrap">
                        ...
                        <div className="text-right flex-shrink-0">
                          <div className="text-xs font-bold text-slate-600">{assignment.points} pts</div>
                          {assignment.grade !== null && (
                            <div className="text-lg font-black text-emerald-600">{assignment.grade}/{assignment.points}</div>
                          )}
                        </div>
                      </div>
                      ...
                      {assignment.feedback && (
                        <div className="mt-3 bg-blue-50 border border-blue-100 rounded-lg p-2.5 flex items-start gap-2">
                          <MessageSquare size={13} className="text-blue-500 flex-shrink-0 mt-0.5" />
                          <p className="text-xs text-blue-700 leading-relaxed">{assignment.feedback}</p>
                        </div>
                      )}
