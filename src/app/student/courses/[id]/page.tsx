  ...
  return (
    <div className="pb-24">
      ...
      >
        ...
        <Link href="/student/courses" className="inline-flex items-center gap-1.5 text-white/80 text-sm mb-4 hover:text-white transition-colors select-none">
          <ArrowLeft size={16} />
          Back to Courses
        </Link>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <TabsList className="bg-white border border-slate-200 rounded-xl p-1 mb-5">
                <TabsTrigger
                  ...
                  style={activeTab === "modules" ? { backgroundColor: course.color, color: "#ffffff" } : {}}
                >
                  Modules
                </TabsTrigger>
                <TabsTrigger
                  value="overview"
                  className="rounded-lg text-xs font-semibold transition-all"
                  style={activeTab === "overview" ? { backgroundColor: course.color, color: "#ffffff" } : {}}
                >
                  Overview
                </TabsTrigger>
                <TabsTrigger
                  value="materials"
                  className="rounded-lg text-xs font-semibold transition-all"
                  style={activeTab === "materials" ? { backgroundColor: course.color, color: "#ffffff" } : {}}
                >
                  Materials
                </TabsTrigger>
              </TabsList>
                    ...
                    {course.modules_list.map((mod) => (
                      <AccordionItem
                        ...
                        className={cn(
                          ...
                          mod.inProgress && "ring-1"
                        )}
                        style={mod.inProgress ? { borderColor: course.color + "60", boxShadow: `0 0 0 1px ${course.color}30` } : {}}
                      >
                        <AccordionTrigger
                          className="px-4 py-3 hover:no-underline hover:bg-slate-50"
                          style={mod.inProgress ? { backgroundColor: course.color + "0D" } : {}}
                        >
                          <div className="flex items-center gap-3 text-left w-full">
                            <div
                              className={cn(
                                "w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0",
                                mod.completed ? "bg-emerald-100" : "bg-slate-100"
                              )}
                              style={mod.inProgress && !mod.completed ? { backgroundColor: course.color + "20" } : {}}
                            >
                              {mod.completed ? (
                                <CheckCircle2 size={16} className="text-emerald-600" />
                              ) : mod.inProgress ? (
                                <Play size={14} style={{ color: course.color }} />
                              ) : (
                            ...
                            <div className="flex-1">
                              <div
                                className={cn(
                                  ...
                                  mod.completed ? "text-slate-500" : "text-slate-700"
                                )}
                                style={mod.inProgress && !mod.completed ? { color: course.color } : {}}
                              >
                            ...
                            {mod.inProgress && (
                              <Badge className="text-[10px] text-white border-none" style={{ backgroundColor: course.color }}>
                                In Progress
                              </Badge>
                            )}
                            {mod.completed && (
                              ...
                              <div key={i} className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 cursor-pointer group">
                                <div className={cn(
                                  ...
                                  {mod.completed ? (
                                    ...
                                    <Play size={12} className="text-slate-400 group-hover:text-slate-700" />
                                  )}
                                </div>
              ...
              <TabsContent value="materials">
                <div className="bg-white rounded-[14px] border border-slate-100 p-5 space-y-3">
                  {["Course Syllabus.pdf", "Reference Handbook.pdf", "Lab Manual.pdf", "Assessment Guide.pdf"].map((file) => (
                    <div key={file} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl hover:bg-slate-100 transition-colors cursor-pointer group">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-red-100 rounded-lg flex items-center justify-center">
                          <span className="text-[10px] font-bold text-red-600">PDF</span>
                        </div>
                        <span className="text-sm text-slate-700 group-hover:text-slate-900 transition-colors">{file}</span>
                      </div>
                      <Download size={15} className="text-slate-400 group-hover:text-slate-600 transition-colors" />
                    </div>
                  ))}
                </div>
              </TabsContent>
